...
def main(config: dict = {}) -> int:
    ...
    flags_specified: bool = any (bool(value) for value in vars(args).values())
    try:
        config = get_config(main_file = __file__, args = args, quiet = flags_specified) if not bool(config) else config
        if   args.get_firmware_version: get_firmware_version(config = config, quiet = False)
            ...
            if confirm_imd_config(config, ordered_api_calls) and wait_for_ping(config):
                if not prompt_to_upgrade_imd_firmware(config = config, quiet = False):
